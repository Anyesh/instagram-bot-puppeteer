...
const sub_reddit = "https://www.reddit.com/r/dankmemes/hot.json";
  ...
  if (previousSession) {
    ...
    if (cookiesArr.length !== 0) {
      for (let cookie of cookiesArr) {
        await page.setCookie(cookie); // set cookies in puppeteer
      }
      console.log("cookies loaded in\n");
    }
  }
}
...
async function upload(num) {
  const selectors = {
    ...
    expand:
      "#react-root > section > div.gH2iS > div.N7f6u.Bc-AD > div > div > div > button.pHnkA",
    wait:
      "#react-root > section > nav.NXc7H.f11OC > div > div > div.KGiwt > div > div > div:nth-child(1) > a > span",
  };

  const browser = await puppeteer.launch();
  const page = await browser.newPage();
  await page.emulate(iphone); // emulate mobile version of the website.
  ...
  try {
    // this checks if you can zoom out the picture or not
...
async function bot() {
  ...
  if (dwnld == false) {
    await download();
    console.log("memes downloaded.");
    dwnld = true;
  }
  for (const item of hour) {
    console.log("hour and picture", picNum);
    await upload(picNum);
    picNum++;
